...
caex(1:ie,1:jb,1:58)=ca(2);     
cbex(1:ie,1:jb,1:58)=cb(2);

caey(1:ib,1:je,1:58)=ca(2);
cbey(1:ib,1:je,1:58)=cb(2);

caez(1:ib,1:jb,1:58)=ca(2);
cbez(1:ib,1:jb,1:58)=cb(2);


dahx(1:ib,1:je,1:58)=da(2);
dbhx(1:ib,1:je,1:58)=db(2);

dahy(1:ie,1:jb,1:58)=da(2);
dbhy(1:ie,1:jb,1:58)=db(2);

dahz(1:ie,1:je,1:58)=da(2);
dbhz(1:ie,1:je,1:58)=db(2);
...
for k=2:kebc         %%%%                      % 与sigmaz有关的量
  ...
  caexybcd(1:ie,1:jb,k)=ca(2);
  cbexybcd(1:ie,1:jb,k)=cb(2);  
  caeyxbcd(1:ib,1:je,k)=ca(2);                   
  cbeyxbcd(1:ib,1:je,k)=cb(2);  
  dahzxbcd(1:ie,1:je,k)=da(2);
  dbhzxbcd(1:ie,1:je,k)=db(2);  
  dahzybcd(1:ie,1:je,k)=da(2);
  dbhzybcd(1:ie,1:je,k)=db(2);   
end
...
for k=1:kebc                               % 与sigmazs有关的量
  ...
  caezxbcd(1:ib,1:jb,k)=ca(2);              %与sigmaz、sigmazs无关的量      
  cbezxbcd(1:ib,1:jb,k)=cb(2);  
  caezybcd(1:ib,1:jb,k)=ca(2);                   
  cbezybcd(1:ib,1:jb,k)=cb(2);   
  dahxybcd(1:ib,1:je,k)=da(2);  
  dbhxybcd(1:ib,1:je,k)=db(2);  
  dahyxbcd(1:ie,1:jb,k)=da(2);
  dbhyxbcd(1:ie,1:jb,k)=db(2); 
end
...
ex(1:ie,2:je,58)=caex(1:ie,2:je,58).*ex(1:ie,2:je,58)+...     
           cbex(1:ie,2:je,58).*(hz(1:ie,2:je,58)-hz(1:ie,1:je-1,58)-...
                                ...
                                2*dt*Jx(1:ie,2:je,58)/((epsz+3*epsz));
...
ey(2:ie,1:je,58)=caey(2:ie,1:je,58).*ey(2:ie,1:je,58)+... 
           cbey(2:ie,1:je,58).*(hx(2:ie,1:je,58)-hx(2:ie,1:je,57)-...
                              hz(2:ie,1:je,58)+hz(1:ie-1,1:je,58))-...
                                2*dt*Jy(2:ie,1:je,58)/((epsz+3*epsz));                            
...
ex(1:ie,jb,58)=caex(1:ie,jb,58).*ex(1:ie,jb,58)+...
           cbex(1:ie,jb,58).*(hz(1:ie,1,58)-hz(1:ie,je,58)-...   
                                ...
                                2*dt*Jx(1:ie,jb,58)/((epsz+3*epsz));
...
ey(ib,1:je,58)=caey(ib,1:je,58).*ey(ib,1:je,58)+...
           cbey(ib,1:je,58).*(hx(ib,1:je,58)-hx(ib,1:je,57)-...
                              hz(1,1:je,58)+hz(ie,1:je,58))-...
                                2*dt*Jy(ib,1:je,58)/((epsz+3*epsz));
...
%exzbct(:,1:jb,1)=caexzbct(:,1:jb,1).*exzbct(:,1:jb,1)-...
%  cbexzbct(:,1:jb,1).*(hyzbct(:,1:jb,1)+hyxbct(:,1:jb,1)-...
%                                 hy(:,:,ke));
...
%eyzbct(:,:,1)=caeyzbct(:,:,1).*eyzbct(:,:,1)+...
%  cbeyzbct(:,:,1).*(hxybct(:,:,1)+hxzbct(:,:,1)-...
%                             hx(:,:,ke));
...
%hy(:,:,100)=hy(:,:,100)-dt*ex(:,:,100)/(muz*ds);                             
